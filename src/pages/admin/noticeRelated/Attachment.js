...
//     useEffect(() => {
//         dispatch(getAllNotices(currentUser?.school?._id || currentUser?._id, "Notice"));
//     }, [dispatch, currentUser?._id, currentUser?.school?._id, forceUpdate]);
...
//             setNewNotice({ title: '', details: '', files: [] });
//             setSelectedFiles([]);
//             setForceUpdate(prev => !prev); // Force re-fetch of notices
//         });
//     };
...
//                             </Typography>
//                             <Typography variant="body2" color="textSecondary" sx={{ maxWidth: '500px' }}>
//                                 There are no notices yet. {isAdmin && "Click the plus button to add a new notice."}
//                             </Typography>
//                             {isAdmin && (
//                                 <Button
//                                     variant="contained"
...
//             {/* Add New Notice Dialog (only for admin) */}
//             {isAdmin && (
//                 <Dialog 
//                     open={openDialog} 
//                     onClose={() => setOpenDialog(false)} 
...
const ShowNotices = () => {
    ...
    useEffect(() => {
        ...
        if (currentUser?.role === 'Admin') {
            ...
            // For Teachers and Students, fetch by school ID
            dispatch(getAllNotices(currentUser?.school?._id || currentUser?.school, "Notice"));
        }
    }, [dispatch, currentUser, forceUpdate]);
    ...
    const handleSubmit = () => {
        ...
        if (currentUser.role === 'Admin') {
            ...
            formData.append('schoolID', currentUser.school._id || currentUser.school);
        }
    ...
    const canAddNotices = ['Admin', 'Teacher'].includes(currentUser?.role);
    const isAdmin = currentUser?.role === 'Admin';
    ...
    return (
        <Box sx={{ padding: { xs: 1, md: 3 }, maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <>
                    {noticesList.length > 0 ? (
                        ...
                        <Box sx={{ 
                            ...
                            <Typography variant="body2" color="textSecondary" sx={{ maxWidth: '500px' }}>
                                {canAddNotices ? "There are no notices yet. Click the plus button to add a new notice." : "There are no notices available at this time."}
                            </Typography>
                            {canAddNotices && (
                                <Button
                                    variant="contained"
            ...
            {/* Add New Notice Dialog (only for admins and teachers) */}
